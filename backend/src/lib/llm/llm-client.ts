...
import type { LlmProviderName, LlmConfig } from './llm.config.js';
...
// Google Gemini via its OpenAI-compatibility endpoint (Bearer key, same
// /chat/completions + response_format contract) — no separate adapter needed.
class GeminiLlmClient extends BaseOpenAiCompatibleClient {
  readonly name = 'gemini' as const;
}

// OpenRouter is OpenAI-compatible; the model id carries the vendor prefix
// (e.g. "openai/gpt-4o-mini", "google/gemini-2.0-flash").
class OpenRouterLlmClient extends BaseOpenAiCompatibleClient {
...
export function createLlmClient(config: LlmConfig): LlmClient | null {
  ...
  switch (config.provider) {
    ...
    case 'gemini':
      return new GeminiLlmClient(apiKey, model, baseUrl, timeoutMs, maxTokens);
    case 'openrouter':
      return new OpenRouterLlmClient(apiKey, model, baseUrl, timeoutMs, maxTokens);
    case 'anthropic':
