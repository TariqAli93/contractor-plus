/**
 * backendChecker.cjs — health + version handshake against the local backend.
 *
...
async function checkHealth(port = rt.backendPort()) {
...
async function checkVersion(port = rt.backendPort()) {
...
async function waitUntilHealthy({ port = rt.backendPort(), retries = 60, intervalMs = 1000, logger } = {}) {
 ...
 */
async function ensureBackendReady({ manager, logger } = {}) {
...
module.exports = { checkHealth, checkVersion, protocolOk, waitUntilHealthy, ensureBackendReady };
